...
class CompiledJaxpr(NamedTuple):
  jaxpr: Jaxpr
  consts: Tuple[Any]

  def __hash__(self) -> int:
    return hash((id(self.jaxpr), *self.consts))

  def __eq__(self, other: CompiledJaxpr):
    if type(other) is not CompiledJaxpr:
      return False
    if id(self.jaxpr) != id(other.jaxpr):
      return False
    return np.array_equal(self.consts, other.consts)

  def execute(self, input_bufs):
    args = list(self.consts) + list(input_bufs)
    out = eval_jaxpr(self.jaxpr, args)
    return out

@lru_cache()
def xla_callable(hashable_jaxpr: IDHashable, hashable_consts: Tuple[IDHashable]):
  jaxpr, consts = hashable_jaxpr.val, list(x.val for x in hashable_consts)
  jaxpr: Jaxpr
  compiled = CompiledJaxpr(jaxpr, tuple(consts))
...
def execute_compiled(compiled: CompiledJaxpr, out_avals, *args):
...
class DeviceArray:
  ...
  def __array__(self): return to_py(self.buf)
  def __repr__(self):  return repr(to_py(self.buf))
  def __str__(self):   return str(to_py(self.buf))
