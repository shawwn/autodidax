...
from autodidax._src import lib
from autodidax._src.lib import dax_jit
...
class Config:
  ...
  def parse_flags_with_absl(self):
    ...
    if not already_configured_with_absl:
      # Extract just the --dax... flags (before the first --) from argv. In some
      # environments (e.g. ipython/colab) argv might be a mess of things
      # parseable by absl and other junk.
      dax_argv = itertools.takewhile(lambda a: a != '--', sys.argv)
      dax_argv = ['', *(a for a in dax_argv if a.startswith('--dax'))]

      import absl.flags
      self.config_with_absl()
      absl.flags.FLAGS(dax_argv, known_only=True)
      self.complete_absl_config(absl.flags)
      already_configured_with_absl = True

      if not FLAGS.dax_omnistaging:
        raise Exception(
          "Disabling of omnistaging is no longer supported in DAX version 0.2.12 and higher: "
          "see https://github.com/google/dax/blob/main/design_notes/omnistaging.md.\n"
          "To remove this warning, unset the DAX_OMNISTAGING environment variable.")

  def enable_omnistaging(self):
    warnings.warn(
        "enable_omnistaging() is a no-op in DAX versions 0.2.12 and higher;\n"
        "see https://github.com/google/dax/blob/main/design_notes/omnistaging.md")
  ...
  def define_bool_state(
    ...
    Example:
      ...
      # Now the DAX_ENABLE_FOO shell environment variable and --dax_enable_foo
      # command-line flag can be used to control the process-level value of
      # the configuration option, in addition to using e.g.
      # ``config.update("dax_enable_foo", True)`` directly. We can also use a
      # context manager:
    ...
    ``config.dax_enable_foo``. Reading it via ``config.FLAGS.dax_enable_foo`` is
    an error.
    """
    name = name.lower()
  ...
  def define_enum_state(
    ...
    if default is not None and default not in enum_values:
      raise ValueError(f"Invalid value \"{default}\" for DAX flag {name}")
  ...
  def _trace_context(self):
    ...
    return (self.x64_enabled, self.dax_numpy_rank_promotion,
            self.dax_default_matmul_precision)
...
class _StateContextManager:
  def __init__(self, name, help, update_thread_local_hook,
    ...
    self._name = name
    self.__name__ = name[4:] if name.startswith('dax_') else name
...
def update_thread_local_jit_state(**kw):
  tls = dax_jit.thread_local_state()
...
flags.DEFINE_bool(
    'dax_omnistaging',
    bool_env('DAX_OMNISTAGING', True),
    help=('Deprecated. Setting this flag to False raises an error. Setting it '
          'to True has no effect.'),
)

flags.DEFINE_integer(
    'dax_tracer_error_num_traceback_frames',
    int_env('DAX_TRACER_ERROR_NUM_TRACEBACK_FRAMES', 5),
    help='Set the number of stack frames in DAX tracer error messages.'
)

flags.DEFINE_bool(
    'dax_host_callback_inline',
    bool_env('DAX_HOST_CALLBACK_INLINE', False),
    help='Inline the host_callback, if not in a staged context.'
)
flags.DEFINE_integer(
    'dax_host_callback_max_queue_byte_size',
    int_env('DAX_HOST_CALLBACK_MAX_QUEUE_BYTE_SIZE', int(256 * 1e6)),
...
flags.DEFINE_bool(
    'dax_host_callback_outfeed',
    bool_env('DAX_HOST_CALLBACK_OUTFEED', False),
    help=(
...
enable_checks = config.define_bool_state(
    name='dax_enable_checks',
    default=False,
    help='Turn on invariant checking for DAX internals. Makes things slower.')

check_tracer_leaks = config.define_bool_state(
    name='dax_check_tracer_leaks',
    default=False,
...
enable_custom_prng = config.define_bool_state(
    ...
    help=('Enables an internal upgrade that allows one to define custom '
          ...
          'be enabled by default in future versions of DAX, at which point '
...
hlo_source_file_canonicalization_regex = config.define_string_state(
    name='dax_hlo_source_file_canonicalization_regex',
    default=None,
...
def _update_x64_global(val):
  lib.dax_jit.global_state().enable_x64 = val

def _update_x64_thread_local(val):
  lib.dax_jit.thread_local_state().enable_x64 = val
...
config._contextmanager_flags.remove("dax_enable_x64")

Config.x64_enabled = Config.dax_enable_x64  # type: ignore

def _update_disable_jit_global(val):
  lib.dax_jit.global_state().disable_jit = val

def _update_disable_jit_thread_local(val):
  lib.dax_jit.thread_local_state().disable_jit = val

disable_jit = config.define_bool_state(
    name='dax_disable_jit',
    default=False,
...
default_matmul_precision = config.define_enum_state(
    name='dax_default_matmul_precision',
    ...
    help=('Control the default matmul and conv precision for 32bit inputs.\n\n'
          ...
          'operation; for example, see the :func:`dax.lax.conv_general_dilated` '
          'and :func:`dax.lax.dot` docstrings. But it can be useful to control '
...
traceback_filtering = config.define_enum_state(
    name = 'dax_traceback_filtering',
    enum_values=["off", "tracebackhide", "remove_frames", "auto"],
    default="auto",
    help="Controls how DAX filters internal frames out of tracebacks.\n\n"
         "Valid values are:\n"
